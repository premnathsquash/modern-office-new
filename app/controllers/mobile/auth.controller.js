...
exports.resetPassReq = async (req, res) => {
  try {
    ...
    await Profile.findOne({ email: email }, async(err, data) => {
      ...
      await Profile.findOneAndUpdate({ email: email }, {otp: otp}, (err1, data1)=>{
        if (err1) {
  ...
  try {
    ...
    await Profile.findOne({ email: email }, async(err, data) => {
      ...
      if(data._doc.otp === otp ){
        ...
        await Profile.findOneAndUpdate({ email: email }, {password: bcrypt.hashSync(password1, 8)}, async(err1, data1)=>{
          if (err1) {
            return res.status(500).send({ message: err1 });
          }
          await sendMail(
